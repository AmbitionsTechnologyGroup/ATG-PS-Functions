...
#Custom
	...
	If ($DetectedSite){
		...
		[string]$DetectedChoco = $DetectedSite.Choco

		if ($DetectedChoco -eq $null -or $DetectedChoco -eq '') {
			Write-Host "Install Ambitions Chocolatey Server"
			$ChocoName = "ACG-Choco"
			[string]$DetectedChoco = "http://choco.ambitionsgroup.com:8081/repository/chocolatey-group/"
		} else {
		...
		Write-Host "You're not at a detected site"
		Write-Host "Install Ambitions Chocolatey Server"
			$ChocoName = "ACG-Choco"
			[string]$DetectedChoco = "http://choco.ambitionsgroup.com:8081/repository/chocolatey-group/"
	}
	$SourceAdd = "choco source add -n=" + $ChocoName + " -s=" + $DetectedChoco + " --priority=1"
	...
	if ($url -eq $null -or $url -eq '') {
		...
		$url = $url.Replace("https://chocolatey.org/api/v2/package/", $DetectedChoco)
	}
#EndCustom
